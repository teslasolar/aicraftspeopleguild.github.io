#!/usr/bin/env python3
"""
Build L3/db/tags.sqlite + L3/db/sql/seed.sql from every
JSON manifest in the project.

Sources:
  L2/scada/gateway/providers/registry.json   · subsystems
  <subsystem>/udts.json                       · UDTs + fields
  <subsystem>/tags.json                       · tag catalog
  L2/scada/programs/**/*.json                 · program manifests

Outputs:
  L3/db/tags.sqlite   · SQLite binary (committed)
  L3/db/sql/seed.sql  · text dump for diff-readable review
"""
from __future__ import annotations
import json, os, sqlite3, sys
from pathlib import Path

ROOT     = Path(__file__).resolve().parent.parent.parent   # guild/Enterprise
DB_DIR   = ROOT / 'L3' / 'db'
DB_FILE  = DB_DIR / 'tags.sqlite'
SCHEMA   = DB_DIR / 'sql' / 'schema.sql'
SEED_OUT = DB_DIR / 'sql' / 'seed.sql'
REGISTRY = ROOT / 'L2' / 'scada' / 'gateway' / 'providers' / 'registry.json'
...
def ingest_programs(con):
    prog_dir = ROOT / 'L2' / 'scada' / 'programs'
...
def dump_sql(con, out: Path):
    ...
    lines = ['-- AUTO-GENERATED by Enterprise/L3/db/build.py · do not hand-edit.\n',
